...
const CustomsInvoiceGenerator = () => {
  ...
  // USITC API Integration - works most times, maybe there is a limit being hit?
  const fetchHtsData = async () => {
    ...
    try {
      ...
      // If the API fails, fallback to hard-coded values
      // Values are current as of 8 July 2025
      if (!response.ok) {
  ...
  const autoDetectTariffsFromApi = (htsCode, countryOfOrigin) => {
    ...
    if (htsData && htsData.data && Array.isArray(htsData.data)) {
      ...
      if (htsEntry) {
        ...
        // Just a first pass. Will use checkboxes to over-ride if needed
  ...
  // IEEPA tariff affected countries and rates (as of 8 July 2025)
  // Note: China rate set to rise to 34% on 12 August 2025. I am way to lazy to implement a timer. 
  ...
  // Need better descriptions for these. Later. 
  ...
  const commonHtsCodes = {
    ...
    'Furniture - Metal Furniture for Commercial Use': '9403.20.00.90',
    'Furniture - Wooden Furniture for Commercial Use': '9403.60.80.93',
    'Electronics - Sound/Visual Alarm Systems': '8531.10.00.45',
    'Custom/Other - Enter Manual HTS Code': 'CUSTOM'
  };

  // Calculate totals
  // Floating point error here sometimes
  useEffect(() => {
  ...
  const generateInvoice = () => {
    ...
    // Not sure where I will use this, thinking if there's an API will return this data. Mostly focused on the HTML output in the next section so call this WIP for now
